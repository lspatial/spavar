...
myploylayer=readOGR(dsn="PG:host=localhost user=postgres dbname=la_birth password=*  port=9434", layer="tracts_no2_effects") 
...
# Filter out those with NULL for the effect;  sp_no2_ap_mean: health effects of NO2 output from Stage One 
dset=dataSet[!is.na(dataSet$sp_no2_ap_mean),] 
# Formula for NO2 spatial modeling, only the predictors with p-values<0.05 selected according to exploratory test of BayesX  
formulaStr="sp_no2_ap_mean~sx(gid,bs='mrf',map=mybnd) + sx(dist0)+sx(ndvi_t)+
            sx(p_per_tra) + sx(p_wbo_tra)+sx(race_white) + sx(race_black) +sx(race_asian)+ 
            sx(edu_no_female) + sx(edu_bacp_female) + sx(med_h_income)+sx(lu_0_ele) + 
            sx(lu_h_ind)+sx(p_clean_energy)"  
...
#Set of the predictors for influence of NO2 effect across Census tracts 
selcovs=c("dist0","ndvi_t","p_per_tra", "p_wbo_tra","race_white","race_black",
          "race_asian","edu_no_female","edu_bacp_female","med_h_income","lu_0_ele",  
          "lu_h_ind","p_clean_energy")
...
#Make the non-linear association plot and save it into the plot file (png format)   
  ...
  par(mar=c(2,4.3,1,1))    
  ylab = expression (paste("Influence on","  ",NO[2], " effect (g/ppb)"))
...
#Save the effect file into a csv file (no2_inf_effect.csv) 
write.csv(covs_eff_statistics,file="no2_inf_effect.csv",row.names=F)  

#Select the first model for posterior estimates 
abmodel=bmodel[[1]]
#Calculate the variance explained from the complete model selected 
obs_v=dset2$sp_no2_ap_mean; poster_v=fitted(abmodel)
var_explained=cov(obs_v,poster_v)^2/(var(obs_v)*var(poster_v))

#Revise the formula with removal of spatial effect 
formulaStr_nosp="sp_no2_ap_mean~ sx(dist0)+sx(ndvi_t)+
                  sx(p_per_tra) + sx(p_wbo_tra)+sx(race_white) + sx(race_black) +sx(race_asian)+ 
                  sx(edu_no_female) + sx(edu_bacp_female) + sx(med_h_income)+sx(lu_0_ele) + 
                  sx(lu_h_ind)+sx(p_clean_energy)"  
...
obs_v=dset2$sp_no2_ap_mean; poster_v=fitted(abmodel_nosp)
...
poster_estimates=data.frame(tract=dset$tract,observed=dset2$sp_no2_ap_mean,post_est=final_fitted[[1]]) 
...
write.csv(poster_estimates,file="poster_estimates_no2.csv",row.names=F) 
