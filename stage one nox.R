...
# Definition of covariates: (1) brrawid_integ: unique id of the subject location (integer type); (2) sp_nox: NOx exposure for the entire pregnancy period;
# (3) age_mother: age of the pregnant mother; (4) ndvi100: NDVI with the 100 m buffer radius;
# (5) birth_weight: term birth weight (unit: g); (6) gest_len: length of gestation; (7) ethnicity; 
# (8) edu_lev: educational level; (9) parity; (10) p_hea_c: primary health care; (11) int_gen: infant's gender.  
all_subs2=na.omit(all_subs[,c("brrawid_integ","sp_nox","age_mother","ndvi100",
...
gm1=gam(birth_weight~sp_nox+factor(edu_lev)+factor(ethnicity)+factor(parity)+ factor(p_hea_c)+s(age_mother_1)+s(ndvi100_1)+s(gest_len_1), data = all_subs)   
# Extract the statistics (mean and precision) of the intercept from the GAM modeling for the whole sample, as priori knowledge for JAGS modeling 
sm=summary(gm1)  
...
for(i in 1:nrow(tracts)){ 
  ...
  tr_subs2=na.omit(tr_subs[,c("brrawid_integ","sp_nox","age_mother","ndvi100",
                              "birth_weight","gest_len","edu_lev","ethnicity","parity", "p_hea_c")])
  ...
  jd=try(jagam(sp_nox~factor(edu_lev)+factor(ethnicity)+factor(parity)+factor(p_hea_c)
  ...
  # Since no priori knowledge for NOx, so no priori knowledge supplied 
...
write.csv(tracts_res,file="tracts_nox_effects.csv",row.names=FALSE) 
